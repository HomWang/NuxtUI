<template>
  <div :id="id" ref="checkbox-group" :class="currentClass">
    <div v-if="selectAll" :class="selectAllClass">
      <t-checkbox
        :id="`indeterminate`"
      ...
      />
      <label :class="labelClass" :for="`indeterminate`">{{
        selectAllLabel
      }}</label>
    </div>
    ...
    >
      ...
      <label :class="labelClass" :for="`${id || name || 'option'}-${index}`">{{
        option.text
      }}</label>
    </div>
  </div>
...
import TCheckboxGroupTheme from "../plugins/themes/default/TCheckboxGroup";
