<template>
  <table v-if="ready" :class="currentClass">
    <slot
      ...
      name="thead"
    >
      <thead v-if="showHeader" :class="theadClass.thead">
        <tr :class="theadClass.tr">
          <th
    ...
    >
      <tbody :class="tbodyClass.tbody">
        <slot
          ...
          name="row"
        >
          <tr :key="rowIndex" :class="tbodyClass.tr">
            <slot
              ...
              name="column"
            >
              <td :key="`${rowIndex}-${columnIndex}`" :class="tbodyClass.td">
                {{ text }}
              </td>
            </slot>
          </tr>
    ...
    <slot
      ...
      name="tfoot"
    >
      <tfoot v-if="showFooter" :class="tfootClass.tfoot">
        <tr :class="tfootClass.tr">
          <td
...
import TTableTheme from "../plugins/themes/default/TTable";
import pick from "lodash/pick";

const { tableClass, theadClass, tbodyClass, tfootClass } = TTableTheme;
  ...
  computed: {
    renderResponsive() {
      return (
        this.responsive &&
        this.windowWidth &&
        this.windowWidth < this.responsiveBreakpoint
      );
    },
