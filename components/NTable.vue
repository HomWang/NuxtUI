...
export default {
  ...
  props: {
    data: { type: Array, default: () => { return [] } }, //数据
    height: { type: String, default: () => { return 'normal' } }, //高度
    maxHeight: { type: String, default: () => { return 'normal' } }, //最高 高度
  ...
  methods: {
    ...
    createHead (createElement) {
      ...
      NTableColumnList.map(item => {
        renderThList.push(createElement(TAGTH,
          {
            ...
            [
              NTableColumnList.map((item) => {
                ...
                return createElement(TAGTD,
                  ...
                  [
                    data.scopedSlots ? data.scopedSlots.default(row) : createElement(TAGNTABLECOLUMN, this.data[i][item.data.attrs.prop])
                  ]
                );
              })
            ]
    ...
    createFooter (createElement) {
      if (this.$slots.footer) {
        return createElement(TAGP, { attrs: { class: this.getFooterClass } }, this.$slots.footer)
      } else {
        return ""
      }
    },
     ...
     */
    getNTableColumn () {
      return this.$slots.default.filter(item => { if (item.tag === NTABLECOLUMN) { return item } });
    },
    /**
     * 获取列数据
     */
